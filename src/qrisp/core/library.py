...
from qrisp import cx, control, ry
import numpy as np
...
import numpy as np
from qrisp import QuantumVariable, amplitude_amplification, cx
...
def acc_QAE(qargs,state_function, oracle_function, eps, alpha, kwargs_oracle = {}):
    ...
    while break_cond >= 2 * eps : 
        ...
        n_i = quantCirc( int((K_i -1 )/2) , N_i, qargs,state_function, 
...
def quantCirc(k,N, qargs,state_function, 
    ...
    res_dict = qbl.get_measurement(shots = N)
    qbl.uncompute()

    # case of single dict return, this should not occur but to be safe
    if len(res_dict.values()) ==1:
        sec_key = 1 - int(list(res_dict.keys())[0])
        res_dict.setdefault(str(sec_key), 0)
...
def compute_thetas(m_i, K_i, A_i, E): 
    ...
    # both starting results should be negative to cover the sin^2 and arcsin symmetries
    b_intermed = np.arcsin( np.sqrt(b_max) ) - np.pi
    b_intermed2 = np.arcsin( - np.sqrt(b_max) )
    condb = True
    while condb :

        if not (m_i*np.pi/2 <= b_intermed <= (m_i+1)*np.pi/2):
            b_intermed +=   np.pi

        else: 
            thetab_intermed = b_intermed
            condb = False
            break

        if not (m_i*np.pi/2 <= b_intermed2 <= (m_i+1)*np.pi/2):
            b_intermed2 +=  np.pi

        else: 
            thetab_intermed = b_intermed2
            
            condb = False
            break
    
    theta_b = thetab_intermed/K_i


    sh_intermed = np.arcsin( np.sqrt(sh_min)) - np.pi
    sh_intermed2 = np.arcsin( - np.sqrt(sh_min))
    sh_cond = True
    while sh_cond :
        # what happens if i do -=??
        if not (m_i*np.pi/2 <= sh_intermed <= (m_i+1)*np.pi/2):
            sh_intermed +=   np.pi
        else: 
            sh_theta_intermed = sh_intermed
            sh_cond = False
            break

        if not (m_i*np.pi/2 <= sh_intermed2 <= (m_i+1)*np.pi/2):
            sh_intermed2 +=   np.pi
        else: 
            sh_theta_intermed = sh_intermed2
            sh_cond = False
            break
